...
public final class GameServerActionImpl implements
		GameServerAction {
	...
	public void addLogicServerRPC(List<ServerInfo> logicServerInfos,String serverId) throws Exception {
		...
		for(ServerInfo serverInfo : logicServerInfos){
			RPCClient client = new RPCClient(new InetSocketAddress(serverInfo.getHost(),
						serverInfo.getPort()), serverInfo.getServerId(), Application.LOGIC_SERVER);
			...
			for(Map.Entry<String, Class<?>> e : map.entrySet()){
				RPCKernel.addServerIDRemote(serverInfo.getServerId(),
	...
	public void addLogicServerRPC(ServerInfo logicServerInfo) throws Exception {
		RPCClient client = new RPCClient(new InetSocketAddress(logicServerInfo.getHost(),
				logicServerInfo.getPort()), logicServerInfo.getServerId(), Application.LOGIC_SERVER);
		RPCKernel.addRPCClient(client);
	...
	@Override
	public void sendMessage(String sessionId, Head head, Object... args) {
		Player player = PlayerManager.getOnlinePlayer(sessionId);
		if(player!=null){
			Application.sendMessage(player.getGameHandler().getChannel(), head, args);
		}
	}

	@Override
	public void sendMessage(List<String> sessionIds, Head head, Object... args) {
		for(String sessionId : sessionIds){
			sendMessage(sessionId, head, args);
		}
	}

	@Override
	public void sendChatChannelMessage(int channelId, Head head, Object... args) {
		Application.sendGroupMessage(PlayerManager.getChatChannel(channelId), head, args);
	}
}
