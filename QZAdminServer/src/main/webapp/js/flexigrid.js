			...
			addData: function (data) { //parse data
				...
				if (p.dataType == 'json') {
					$.each(data.rows, function (i, row) {
						...
						if(typeof p.getRowColor == "function"){
							jtr.css('background', p.getRowColor(row));
						}else if (row.color) {
						...
						$('thead tr:first th', g.hDiv).each( //add cell
							function () {
								...
								if(p.colModel[idx].checkbox){
									td.innerHTML = "<input type='checkbox' name='"+p.colModel[idx].name+"' value='" + row[p.colModel[idx].name]+"'/>";
								}else{
									...
									if(typeof p.colModel[idx].customValue == "function"){
										td.innerHTML = p.colModel[idx].customValue(i, idx, td.innerHTML, p.colModel[idx].name);
									}
								}
		...
		if (p.colModel) { //create model if any
			...
			for (var i = 0; i < p.colModel.length; i++) {
				...
				if( cm ) {	// only use cm if its defined
					...
					if (cm.customValue) {
						th.customValue = cm.customValue;
					}
					if(cm.checkbox){
						$(th).attr('checkbox',cm.checkbox);
						var cbox = document.createElement("input");
						$(cbox).attr("type", "checkbox").attr("name", "fleigrid_t_" + cm.name).attr("value", cm.name);
						$(th).append(cbox);
					}
				} else {
					th.innerHTML = "";
		...
		$('thead tr:first th', g.hDiv).each(function () {
			...
			if($(this).attr('checkbox')){
				$(this).find("input[type='checkbox']").live("click", function(){
					$("input[name='" + $(this).val() + "']").attr("checked", this.checked);
				});
			}
			if (this.hidden) {
				$(this).hide();
