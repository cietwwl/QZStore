...
<div id="search_div" style="display:table;">
	<span style="float:left;">
		<select id="consumeType" style="padding:2px;">
...
<table id="eternal_log_table"></table>
<!-- 武魂日志 -->
...
function ready(){
	...
	$("#eternal_log_table").flexigrid({
		url : 'game_log_mgr/get_game_eternal_log_list.jsp',
   	    dataType : 'json',
...
function searchMoneyLog(){
	...
	$("#eternal_log_table").flexOptions({
		params:params,
		newp:1
	});
	$("#eternal_log_table").flexReload();
}
function openDownloadDialog(){
	...
	$("#download_div").dialog({
		...
		buttons:{
			...
			"确定" : function(){
				var p = {};
				p.begin = $("#downbegintime").datepicker("getDate");
				p.end = $("#downendtime").datepicker("getDate");
				if(p.end.getTime() < p.begin.getTime()){
					alert("结束日期不能小于开始日期");
					return;
				}
